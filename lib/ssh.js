...
function createSshUser({ username, password, expireDays = 30, ipLimit = 1, note = '' }) {
  ...
  db.addLog('CREATE_USER', `Created user ${username} (Expiry: ${expiryDate}, Limit: ${ipLimit} IP)`);
  return newUser;
}

// Renew SSH account
function renewSshUser(username, additionalDays) {
  ...
  db.addLog('RENEW_USER', `Renewed ${username} for +${additionalDays} days (New Expiry: ${newExpiry})`);
